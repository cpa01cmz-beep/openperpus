...
export default function EditorialHero({ banners, siteName, tagline }: HeroProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className="flex flex-col justify-center border-t border-[var(--ink)]/10 p-6 sm:p-8 lg:border-l lg:border-t-0"
      ...
      >
        ...
        {active.subtitle ? (
          <p className="mt-2 max-w-prose text-sm leading-relaxed text-[var(--ink)] opacity-80 sm:text-base">
            {active.subtitle}
          </p>
        ) : null}
        ...
        {total > 1 && (
          <div className="mt-5 flex items-center gap-2">
            <button
              ...
              className="grid min-h-[44px] min-w-[44px] place-items-center rounded-[var(--radius-md)] border border-[var(--ink)]/15 text-[var(--ink)] transition hover:bg-brand-soft focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand"
            >
            ...
            <div className="ml-2 flex gap-1.5" role="tablist" aria-label="Pilih banner">
              {banners.map((b, i) => (
                ...
                >
                  <span
                    aria-hidden="true"
                    className={`h-1.5 rounded-[var(--radius-sm)] transition-all ${
                      i === idx ? 'w-8 bg-[var(--brand-strong)]' : 'w-3 bg-[var(--ink)]/25'
                    }`}
                  />
                </button>
              ))}
