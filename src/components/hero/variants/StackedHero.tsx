  ...
  return (
    ...
    >
      <div className="relative aspect-[16/9] w-full sm:aspect-[21/8]">
        ...
        {total > 1 && (
          <div className="absolute right-4 top-4 flex gap-2">
            <button type="button" onClick={() => go(-1)} aria-label="Banner sebelumnya" className="grid h-9 w-9 place-items-center rounded-none border-2 border-white bg-black text-white transition hover:bg-[#111110] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent">
              <ChevronLeft className="h-5 w-5" />
            </button>
            <button type="button" onClick={() => go(1)} aria-label="Banner berikutnya" className="grid h-9 w-9 place-items-center rounded-none border-2 border-white bg-black text-white transition hover:bg-[#111110] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent">
      ...
      <div className="border-t-2 border-[#111110] p-5 sm:p-8">
        ...
        {active.subtitle ? (
          <p className="mt-2 max-w-xl font-body text-sm text-brand-soft sm:text-base">{active.subtitle}</p>
        ) : null}
            ...
            <div className="flex gap-1.5" role="tablist" aria-label="Pilih banner">
              {banners.map((b, i) => (
                <button
                  ...
                  onClick={() => setIdx(i)}
                  className={`h-2 rounded-none border border-[#111110] transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent ${
                    i === idx ? "w-8 bg-accent" : "w-4 bg-white hover:bg-accent"
                  }`}
                />
              ))}
            </div>
