 ...
 */
export default function SplitHeader({ siteName, tagline, settings, logo_url, logoUrl }: HeaderVariantProps) {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-brand-strong/10 bg-[var(--surface)]/90 backdrop-blur">
      <div className="mx-auto flex h-16 w-full max-w-[var(--container)] items-center justify-between gap-3 px-4 sm:px-6 lg:px-8">
        <Link href="/" className="flex min-w-0 items-center gap-2.5 rounded-[var(--radius-md)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand">
          ...
          <span className="min-w-0 leading-tight">
            ...
            {tag ? <span className="block truncate text-[11px] text-[var(--ink)]/70">{tag}</span> : null}
          </span>
        </Link>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="grid h-10 w-10 place-items-center rounded-[var(--radius-md)] border border-[var(--ink)]/10 text-[var(--ink)] transition hover:bg-brand-soft focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand lg:hidden"
          >
      ...
      {open && (
        <div id="menu-split" className="border-t border-[var(--ink)]/10 bg-[var(--surface)] lg:hidden">
