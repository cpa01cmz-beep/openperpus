...
import type { LibrarySettings, Banner, Article, Testimonial, PageDoc, Book, Category } from "@/lib/types";
...
export async function fetchBooks(opts: FetchBooksOpts = {}): Promise<Book[]> {
  return unstable_cache(
    () => fetchBooksUncached(opts),
    ["books", stableKey(opts)],
    { tags: [BOOKS_TAG], revalidate: 60 }
  )();
}
  ...
  try {
    ...
    switch (opts.sort ?? "terbaru") {
      ...
      case "rating":
        ...
        break;
      case "stok":
        query = query.order("stock_available", { ascending: false });
        break;
      default:
...
export async function fetchBooksPaged(opts: FetchBooksOpts = {}): Promise<PagedBooks> {
  return unstable_cache(
    () => fetchBooksPagedUncached(opts),
    ["books-paged", stableKey(opts)],
    { tags: [BOOKS_TAG], revalidate: 60 }
  )();
}
...
export async function fetchPage(slug: string): Promise<PageDoc | null> {  return unstable_cache(() => fetchPageUncached(slug), ["page-slug", slug], {
    tags: [PAGES_TAG],
    revalidate: 60,
  })();
}
...
async function fetchStatsUncached(): Promise<{
  ...
  try {
    ...
    const [books, cats, arts, copies] = await Promise.all([
      ...
      supabase.from("books").select("stock_total").eq("is_active", true).order("stock_total", { ascending: false }).limit(5000),
    ]);
    const totalCopies =
      copies.data?.reduce((s: number, b: { stock_total?: number | string | null }) => s + (Number(b.stock_total) || 0), 0) ?? 0;
    return {
