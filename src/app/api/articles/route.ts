...
export async function GET(req: Request) {
  ...
  let query = supabase.from("articles").select("*", { count: "exact" }).order("created_at", { ascending: false }).range(from, to);
  if (q) {
...
export async function POST(req: Request) {
  ...
  if (error) {
    if ((error as { code?: string }).code === "23505") return jsonError("CONFLICT", "Slug sudah dipakai.", 409, error.message);
...
export async function PUT(req: Request) {
  ...
  if (body.cover_url !== undefined || body.gambar_url !== undefined) payload.cover_url = (body.cover_url ?? body.gambar_url) as string | null;
  ...
  if (typeof payload.title === "string" && payload.title) {
    payload.slug = typeof body.slug === "string" && body.slug.trim() ? slugify(body.slug) : slugify(payload.title as string);
