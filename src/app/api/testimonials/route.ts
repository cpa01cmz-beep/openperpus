...
export function checkTestimonialRateLimit(
  ip: string,
  now = Date.now()
...
export async function GET(req: Request) {
  ...
  if (error) {
    log.error('testimonials.fetch_failed', { detail: error.message });
    return jsonError('FETCH_FAILED', 'Gagal mengambil testimoni.', 500, {
...
export async function POST(req: Request) {
  ...
  const supabase = staff
    ? (staff as { supabase: ReturnType<typeof createClient> }).supabase
    : createClient();
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      content,
      avatar_url:
        ((body.avatar_url ?? body.foto ?? body.image_url ?? body.gambar_url) as string | null) ??
        null,
      rating,
      sort_order,
  ...
  if (error) {
    ...
    return jsonError('SAVE_FAILED', 'Gagal menambah testimoni.', 500, { requestId: log.requestId });
  }
  ...
  await writeLog(supabase, staffUser?.id, 'testimonials.create', (data as { id: string }).id, {
    name,
  });
