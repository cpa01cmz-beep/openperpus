...
export async function GET(req: Request) {
  ...
  if (overdue === "1") query = query.in("status", ["borrowed", "overdue"]).lt("due_at", new Date().toISOString());
  if (q) {
...
export async function POST(req: Request) {
  ...
  if (Number.isNaN(borrowedAt.getTime())) return jsonError("VALIDATION", "borrowed_at tidak valid.", 422);
  const dueAt = body.due_at ? new Date(body.due_at as string) : new Date(addDaysISO(14, borrowedAt));
  ...
  const { data: member } = await supabase.from("members").select("id,status").eq("id", member_id).single();
  ...
  if (!rpcMissing) {
    ...
    if (/Buku tidak ditemukan/i.test(rpcMsg)) return jsonError("NOT_FOUND", "Buku tidak ditemukan.", 404);
  ...
  const { data: book } = await supabase.from("books").select("id,stock_available").eq("id", book_id).single();
...
export async function PUT(req: Request) {
  ...
  const l = loan as { status: string; due_at: string; book_id: string; member_id: string; returned_at: string | null };
