...
export async function GET(_req: Request, { params }: Ctx) {
  ...
  const { data, error } = await supabase
    ...
    .eq("id", params.id)
    .single();
  if (error) return jsonError("NOT_FOUND", "Buku tidak ditemukan.", 404, error.message);
...
export async function PUT(req: Request, { params }: Ctx) {
  ...
  const copy = ["title", "author", "publisher", "isbn", "year", "category_id", "rack_id", "cover_url", "pdf_url", "description", "pages", "language", "stock_total", "stock_available", "featured", "is_active"] as const;
  ...
  else if (typeof payload.title === "string" && payload.title.trim()) payload.slug = slugify(payload.title);

  if (payload.category_id !== undefined && payload.category_id !== null && !isUuid(payload.category_id)) {
  ...
  if (payload.stock_available !== undefined && (!Number.isInteger(payload.stock_available) || (payload.stock_available as number) < 0)) {
  ...
  if (payload.stock_total !== undefined || payload.stock_available !== undefined) {
    const { data: cur } = await supabase.from("books").select("stock_total,stock_available").eq("id", params.id).single();
    ...
    if (nextAvail > nextTotal) return jsonError("VALIDATION", "stock_available tidak boleh melebihi stock_total.", 422);
  }

  const { data, error } = await supabase.from("books").update(payload).eq("id", params.id).select().single();
  if (error) return jsonError("SAVE_FAILED", "Gagal mengupdate buku.", 500, error.message);
  revalidateTag("books");
  return NextResponse.json({ data });
}
...
export async function DELETE(_req: Request, { params }: Ctx) {
  ...
  if ((count ?? 0) > 0) return jsonError("CONFLICT", "Buku masih dipinjam, tidak bisa dihapus.", 409);
