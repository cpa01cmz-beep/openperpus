...
function revalidated(): string[] {
  const done: string[] = [];
  try { revalidateTag("categories"); done.push("categories"); } catch { /* abaikan */ }
...
export async function GET(_req: Request, { params }: Ctx) {
  ...
  if (error || !data) return jsonError("NOT_FOUND", "Kategori tidak ditemukan.", 404, error?.message);

  // Anon hanya boleh yang aktif; staf boleh semua.
  if ((data as { is_active: boolean }).is_active !== true) {
    const guard = await requireStaff();
    if ("errorResponse" in guard && guard.errorResponse) return jsonError("NOT_FOUND", "Kategori tidak ditemukan.", 404);
  }
...
export async function DELETE(_req: Request, { params }: Ctx) {
  ...
  const { count } = await supabase.from("books").select("id", { count: "exact", head: true }).eq("category_id", params.id);
