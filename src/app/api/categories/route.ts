...
export async function GET(req: Request) {
  ...
  if (error) {
    log.error('categories.fetch_failed', { detail: error.message });
    return jsonError('FETCH_FAILED', 'Gagal mengambil kategori.', 500, {
...
export async function POST(req: Request) {
  ...
  if (error) {
    ...
    return jsonError('SAVE_FAILED', 'Gagal menambah kategori.', 500, { requestId: log.requestId });
  }
