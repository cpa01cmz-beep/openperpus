...
const ALLOWED: Record<string, string> = {
  ...
  name: "name", tagline: "tagline", logo_url: "logo_url", favicon_url: "favicon_url",
  address: "address", phone: "phone", email: "email",
  operational_hours: "operational_hours", socials: "socials",
  welcome_text: "welcome_text", vision: "vision", mission: "mission", about: "about",
  seo_title: "seo_title", seo_desc: "seo_desc",
  announcement: "announcement",
  active_theme: "active_theme",
  // alias Indonesia (docs) -> canonical
  nama: "name", nama_perpus: "name",
  alamat: "address", telepon: "phone",
  jam_operasional: "operational_hours", sosmed: "socials", sosial_media: "socials",
  sambutan: "welcome_text", visi: "vision", misi: "mission",
...
export async function GET() {
  ...
  const { data, error } = await supabase.from("library_settings").select("*").eq("id", 1).maybeSingle();
...
export async function PUT(req: Request) {
  ...
  if (payload.active_theme !== undefined && payload.active_theme !== null && payload.active_theme !== "") {
    const theme = String(payload.active_theme);
    if (!["emerald", "midnight", "paper", "brutalist", "ocean"].includes(theme)) {
      return jsonError("VALIDATION", "Tema tidak dikenal. Pilih emerald, midnight, paper, brutalist, atau ocean.", 422);
    }
  }
  ...
  try { revalidateTag("settings"); } catch { /* abaikan */ }
  try { revalidatePath("/"); } catch { /* abaikan */ }
  return NextResponse.json({ data });
}
