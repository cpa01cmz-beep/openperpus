...
export async function GET(_req: Request, { params }: Ctx) {
  ...
  if (error || !data) return jsonError("NOT_FOUND", "Menu tidak ditemukan.", 404, error?.message);
...
export async function PUT(req: Request, { params }: Ctx) {
  ...
  const { data, error } = await supabase.from("menus").update(payload).eq("id", params.id).select().single();
...
export async function DELETE(_req: Request, { params }: Ctx) {
  ...
  const { count } = await supabase.from("menus").select("id", { count: "exact", head: true }).eq("parent_id", params.id);
