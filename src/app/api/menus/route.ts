...
export async function GET(req: Request) {
  ...
  if (id) {
    ...
    if ((data as { is_active: boolean }).is_active !== true && all !== "1") {
      const guard = await requireStaff();
      if ("errorResponse" in guard && guard.errorResponse) return jsonError("NOT_FOUND", "Menu tidak ditemukan.", 404);
    }
  ...
  if (error) return jsonError("FETCH_FAILED", "Gagal mengambil menu.", 500, error.message);
  const total = count ?? 0;
  return NextResponse.json(
    {
      data,
...
export async function POST(req: Request) {
  ...
  if (error) return jsonError("SAVE_FAILED", "Gagal menambah menu.", 500, error.message);
...
export async function PUT(req: Request) {
  ...
  const { data, error } = await supabase.from("menus").update(payload).eq("id", id).select().single();
  if (error) return jsonError("SAVE_FAILED", "Gagal mengupdate menu.", 500, error.message);
...
export async function DELETE(req: Request) {
  ...
  const { count } = await supabase.from("menus").select("id", { count: "exact", head: true }).eq("parent_id", id);
  if ((count ?? 0) > 0) return jsonError("CONFLICT", "Menu masih punya sub-menu, pindahkan/hapus dulu anaknya.", 409);

  const { error } = await supabase.from("menus").delete().eq("id", id);
  if (error) return jsonError("DELETE_FAILED", "Gagal menghapus menu.", 500, error.message);
