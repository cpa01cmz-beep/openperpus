 ...
 *   — pustakawan+ saja. amount opsional: default lunasi sisa (amount - paid_amount).
...
export async function POST(req: Request, { params }: Ctx) {
  ...
  const method = String(body.metode ?? body.method ?? body.payment_method ?? "cash").trim() || "cash";
  ...
  const f = fine as { amount: number | string; paid_amount: number | string; status: string; member_id: string; loan_id: string };
  ...
  const payAmount = rawAmount === undefined || rawAmount === null || rawAmount === "" ? remain : Number(rawAmount);
  ...
  const { data, error } = await supabase
    ...
    .update({
      ...
      notes: typeof body.notes === "string" && body.notes.trim() ? body.notes.trim() : `Dibayar via ${method}.`,
    })
    .eq("id", params.id)
  ...
  if (error || !data) return jsonError("CONFLICT", "Denda sudah berubah status, muat ulang dulu.", 409, error?.message);
  ...
  try {
    await supabase.from("activity_logs").insert({
      ...
      entity_type: "fines",
      entity_id: params.id,
      metadata: { loan_id: f.loan_id, member_id: f.member_id, amount: payAmount, method, status: (data as { status: string }).status },
    });
  } catch { /* best-effort */ }
