...
function revalidated(): string[] {
  const done: string[] = [];
  try { revalidateTag("racks"); done.push("racks"); } catch { /* abaikan */ }
  try { revalidatePath("/katalog"); done.push("/katalog"); } catch { /* abaikan */ }
  return done;
}
...
export async function GET(_req: Request, { params }: Ctx) {
  ...
  if (error || !data) return jsonError("NOT_FOUND", "Rak tidak ditemukan.", 404, error?.message);

  if ((data as { is_active: boolean }).is_active !== true) {
    const guard = await requireStaff();
    if ("errorResponse" in guard && guard.errorResponse) return jsonError("NOT_FOUND", "Rak tidak ditemukan.", 404);
  }
...
export async function PUT(req: Request, { params }: Ctx) {
  ...
  if (body.location !== undefined || body.keterangan !== undefined || body.lantai !== undefined) {
    ...
    payload.location = v === null || v === undefined || String(v).trim() === "" ? null : String(v).trim();
  }
  if (body.capacity !== undefined) {
    if (body.capacity === null || body.capacity === "") payload.capacity = null;
    else {
      const c = Number(body.capacity);
      if (!Number.isInteger(c) || c < 0) return jsonError("VALIDATION", "capacity harus bilangan bulat >= 0.", 422);
      payload.capacity = c;
    }
  }
  if (body.is_active !== undefined) payload.is_active = Boolean(body.is_active);

  const { data, error } = await supabase.from("racks").update(payload).eq("id", params.id).select().single();
  if (error) {
    if ((error as { code?: string }).code === "23505") return jsonError("CONFLICT", "Kode rak sudah dipakai.", 409, error.message);
    return jsonError("SAVE_FAILED", "Gagal mengupdate rak.", 500, error.message);
  }
...
export async function DELETE(_req: Request, { params }: Ctx) {
  ...
  const { count } = await supabase.from("books").select("id", { count: "exact", head: true }).eq("rack_id", params.id);
  ...
  if (error) return jsonError("DELETE_FAILED", "Gagal menghapus rak.", 500, error.message);
