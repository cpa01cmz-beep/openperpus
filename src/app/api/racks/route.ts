...
export async function GET(req: Request) {
  ...
  if (q) {
    ...
    if (clean) query = query.or(`code.ilike.%${clean}%,name.ilike.%${clean}%,location.ilike.%${clean}%`);
  }

  const { data, error, count } = await query;
  if (error) return jsonError("FETCH_FAILED", "Gagal mengambil rak.", 500, error.message);
  const total = count ?? 0;
...
export async function POST(req: Request) {
  ...
  if (error) {
    ...
    return jsonError("SAVE_FAILED", "Gagal menambah rak.", 500, error.message);
  }
...
export async function PUT(req: Request) {
  ...
  const { data, error } = await supabase.from("racks").update(payload).eq("id", id).select().single();
  if (error) {
    if ((error as { code?: string }).code === "23505") return jsonError("CONFLICT", "Kode rak sudah dipakai.", 409, error.message);
...
export async function DELETE(req: Request) {
  ...
  const { count } = await supabase.from("books").select("id", { count: "exact", head: true }).eq("rack_id", id);
