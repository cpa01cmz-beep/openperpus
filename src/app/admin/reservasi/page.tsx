...
export default function ReservasiPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const json = (await res.json()) as { data?: Reservation[]; pagination?: { totalPages?: number }; meta?: { totalPages?: number } };
  ...
  async function onUpdate(id: string, next: "ready" | "completed" | "cancelled") {
    const label = next === "ready" ? "setujui (siap diambil)" : next === "completed" ? "selesaikan" : "batalkan";
  ...
  return (
    <div className="grid gap-4">
      <div>
        ...
        <p className="text-sm text-slate-500">Setujui reservasi menjadi siap diambil, selesaikan bila buku dipinjam, atau batalkan.</p>
      </div>

      {apiMissing && (
        <div role="alert" className="rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-800">
          API <code className="font-mono">/api/reservations</code> belum tersedia di backend (404). Daftar &amp; aksi
          approve/batal menunggu worker backend. Sudah dilaporkan ke mandor.
        </div>
      )}
      {error && (
              ...
              render: (r) => (
                <span>
                  ...
                  <span className="text-xs text-slate-500">
                    {r.expires_at ? `kadaluarsa ${new Date(r.expires_at).toLocaleDateString("id-ID")}` : "tanpa kadaluarsa"}
                  </span>
                </span>
              ),
            },
