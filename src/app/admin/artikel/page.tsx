...
export default function ArtikelPage() {
  ...
  const [form, setForm] = useState({ title: "", content_md: "", excerpt: "", cover_url: "", category: "", status: "draft" });

  const load = useCallback(async () => {
    const res = await fetch("/api/articles?per_page=50");
  ...
  async function onAdd(e: React.FormEvent) {
    ...
    setForm({ title: "", content_md: "", excerpt: "", cover_url: "", category: "", status: "draft" });
    load();
  }
  ...
  return (
    <div className="grid gap-4">
      ...
      <form onSubmit={onAdd} className="grid max-w-2xl gap-2 rounded-2xl border bg-white p-4">
        <input className={input} placeholder="Judul* (slug otomatis)" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} required />
        ...
        <textarea className={input} rows={4} placeholder="Konten markdown* (content_md)" value={form.content_md} onChange={(e) => setForm({ ...form, content_md: e.target.value })} required />
        <div className="flex gap-2">
      ...
      <DataTable<Article>
        columns={[
          ...
          { key: "aksi", header: "Aksi", render: (r) => (<span className="flex gap-2"><Link href={`/admin/artikel/edit/${r.id}`} className="text-blue-600 hover:underline">Edit</Link><button onClick={() => onDelete(r.id)} className="text-red-600 hover:underline">Hapus</button></span>) },
        ]}
        rows={rows}
        getRowKey={(r) => r.id}
      />
