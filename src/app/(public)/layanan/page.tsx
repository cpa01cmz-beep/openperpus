...
export async function generateMetadata() {
  ...
  return {
    title: `Layanan — ${s.name ?? "Perpustakaan"}`,
    description: `Layanan sirkulasi, keanggotaan, dan fasilitas ${s.name ?? "perpustakaan"}.`,
  };
}
...
export default async function LayananPage() {
  ...
  return (
    <div className="space-y-6">
      <header>
        <p className="text-xs font-bold uppercase tracking-[0.2em] text-accent">Apa yang kami bantu</p>
        <h1 className="mt-1 font-heading text-3xl font-bold text-heading sm:text-4xl">
          Layanan {siteName}
        </h1>
        <p className="mt-2 max-w-2xl text-sm text-slate-500 sm:text-base">
          {page?.excerpt ?? "Seluruh layanan sirkulasi dan informasi — dirender dinamis dari sistem."}
        </p>
      </header>

      {page?.content_md ? (
        <section aria-label="Deskripsi layanan" className="rounded-lg border border-slate-100 bg-white p-6 shadow-sm sm:p-8">
      ...
      <section aria-labelledby="daftar-layanan" className="grid gap-4 sm:grid-cols-2">
        <h2 id="daftar-layanan" className="sr-only">Daftar layanan</h2>
        {DEFAULT_SERVICES.map((s) => (
          <div key={s.title} className="rounded-lg border border-slate-100 bg-white p-5 shadow-sm transition hover:shadow-md sm:p-6">
            <span className="grid h-11 w-11 place-items-center rounded-lg bg-brand/10 text-brand" aria-hidden="true">
      ...
      <section aria-labelledby="alur" className="rounded-lg bg-brand-strong p-6 text-white shadow sm:p-8">
        ...
        <ol className="mt-4 grid gap-3 text-sm sm:grid-cols-3">
          {[
            ...
            <li key={i} className="rounded-lg bg-white/10 p-4">
              <span className="grid h-8 w-8 place-items-center rounded-full bg-accent font-heading text-sm font-bold text-brand-strong" aria-hidden="true">
                {i + 1}
              </span>
