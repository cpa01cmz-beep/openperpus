...
export async function generateMetadata({ params }: Props) {
  ...
  return {
    ...
    alternates: {
      canonical: `${getSiteUrl()}/katalog/${params.slug}`,
    },
  };
}
...
export default async function BookDetailPage({ params }: Props) {
  ...
  const related = book.category_id
    ? (await fetchBooks({ categoryId: book.category_id, limit: 5 })).filter(
        (b) => b.id !== book.id
      ).slice(0, 4)
    : [];
      ...
      <article className="grid gap-6 sm:grid-cols-[240px_1fr] sm:gap-8 lg:grid-cols-[280px_1fr]">
        ...
        <div className="overflow-hidden rounded-[var(--radius-lg)] border border-[var(--ink)]/10 bg-[var(--surface)] shadow-[var(--shadow-sm)]">
          <div className="relative aspect-[3/4] w-full bg-brand-soft">
            {book.cover_url ? (
              ...
              <div className="grid h-full w-full place-items-center bg-gradient-to-br from-brand to-brand p-6 text-center" aria-hidden="true">
                <div>
          ...
          <div className="flex items-center justify-between gap-2 p-4">
            <span className="flex items-center gap-1 text-sm font-semibold text-[var(--ink)]">
              <Star className={`h-4 w-4 ${rating > 0 ? "fill-accent text-accent" : "text-[var(--ink)]/30"}`} aria-hidden="true" />
              {rating > 0 ? `${rating.toFixed(1)} / 5` : "Belum dinilai"}
            </span>
            <span className={`rounded-full border px-3 py-1 text-xs font-bold ${tone}`}>{stock.label}</span>
          </div>
        </div>
        ...
        <div>
          ...
          <dl className="mt-5 grid grid-cols-1 gap-2.5 rounded-[var(--radius-lg)] border border-[var(--ink)]/10 bg-[var(--surface)] p-4 shadow-[var(--shadow-sm)] sm:grid-cols-2">
            {meta.map((m) => (
              <div key={m.label} className="flex items-start gap-2.5 rounded-[var(--radius-md)] bg-[var(--brand-soft)]/60 px-3 py-2.5">
                <m.icon className="mt-0.5 h-4 w-4 shrink-0 text-brand" aria-hidden="true" />
                <div className="min-w-0">
                  <dt className="text-[11px] font-semibold uppercase tracking-wide text-[var(--ink)]/50">{m.label}</dt>
                  <dd className="truncate text-sm font-medium text-[var(--ink)]" title={m.value}>{m.value}</dd>
                </div>
              </div>
            ))}
            <div className="flex items-start gap-2.5 rounded-[var(--radius-md)] bg-[var(--brand-soft)]/60 px-3 py-2.5">
              ...
              <div>
                <dt className="text-[11px] font-semibold uppercase tracking-wide text-[var(--ink)]/50">Stok</dt>
          ...
          <div className="mt-5 flex flex-wrap gap-3" aria-live="polite">
            {available ? (
              <>
                <Link
                  href={`/katalog/${book.slug}?aksi=pinjam`}
                  className="inline-flex items-center gap-2 rounded-[var(--radius-md)] bg-brand px-6 py-3 text-sm font-bold text-white shadow transition hover:bg-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
                >
                  Pinjam Buku <ArrowRight className="h-4 w-4" aria-hidden="true" />
                </Link>
                <Link
                  href={`/katalog/${book.slug}?aksi=reservasi`}
                  className="inline-flex items-center gap-2 rounded-[var(--radius-md)] border border-brand-soft bg-[var(--surface)] px-6 py-3 text-sm font-semibold text-brand shadow-sm transition hover:bg-brand-soft focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand"
                >
                  Reservasi
                </Link>
              </>
              ...
              <>
                <button
                  ...
                  className="cursor-not-allowed rounded-[var(--radius-md)] bg-[var(--ink)]/10 px-6 py-3 text-sm font-bold text-[var(--ink)]/50"
                >
                  Stok Habis
                </button>
                <Link
                  href={`/katalog/${book.slug}?aksi=reservasi`}
                  className="inline-flex items-center gap-2 rounded-[var(--radius-md)] bg-accent px-6 py-3 text-sm font-bold text-brand-strong shadow transition hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand"
                >
