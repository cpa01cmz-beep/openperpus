...
describe("US-1 kontak reservation context: a11y touch targets", () => {
  ...
  it("banner links have focus-visible ring", () => {
    const src = read("src/app/(public)/kontak/page.tsx");
    expect(src.includes("focus-visible:ring"), "banner links must include focus-visible ring").toBe(true);
  });
});
...
describe("US-1 kontak reservation context: slug sanitization / XSS-safe", () => {
  ...
  it("never uses dangerouslySetInnerHTML for the slug/title", () => {
    const src = read("src/app/(public)/kontak/page.tsx");
    expect(
      src.includes("dangerouslySetInnerHTML"),
      "banner must never use dangerouslySetInnerHTML (XSS-safe)"
    ).toBe(false);
  });
  ...
  it("never interpolates raw slug into href without katalog/slug guard", () => {
    ...
    const hasRawHref =
      src.includes("href={buku}") ||
      src.includes("href={slug}") ||
      src.includes("href={raw");
