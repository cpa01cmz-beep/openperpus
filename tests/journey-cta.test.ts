...
describe('S-journey-cta dead CTA never reloads', () => {
  ...
  it('detail has zero dead ?aksi= links', () => {
    ...
    expect(src.includes('?aksi='), 'detail must not contain any ?aksi= dead-end reload').toBe(
      false
    );
  });
  ...
  it('out-of-stock path links to live reservation/contact, never disabled-only', () => {
    ...
    const hasLiveFallback =
      src.includes('/kontak?buku=') ||
      src.includes('href="/reservasi"') ||
      src.includes('href="/kontak"');
    expect(hasLiveFallback, 'out-of-stock must offer live /kontak?buku=slug fallback').toBe(true);
    expect(
      src.includes('Stok Habis'),
      'out-of-stock visible state must keep Stok Habis label'
    ).toBe(true);
  });
});
...
describe('S-journey-cta visible availability state', () => {
  it('detail renders stock count + badge with polite live region', () => {
    ...
    expect(src.includes('tersedia'), 'detail must show visible tersedia availability text').toBe(
      true
    );
    expect(src.includes('aria-live'), 'availability region must be announced via aria-live').toBe(
      true
    );
  });
  ...
  it('BookCard keeps visible stock badge and links to live detail', () => {
    ...
    expect(src.includes('/katalog/${'), 'BookCard must link to live /katalog/[slug] detail').toBe(
      true
    );
...
describe('S-journey-cta empty-state CTA', () => {
  it('detail offers live CTA when no related books', () => {
    ...
    expect(
      src.includes('href="/katalog"'),
      'empty related state must link back to live /katalog'
    ).toBe(true);
  });
...
/** S-JOURNEY-EXT: SearchBar debounce, useDeferredValue, aria-busy, Escape clear */
describe('S-JOURNEY-EXT SearchBar extended UX', () => {
  it('SearchBar implements 300ms debounce on onChange', () => {
    const src = read('src/components/public/SearchBar.tsx');
    // Should use setTimeout/clearTimeout or useDeferredValue for debounce
    const hasDebounce =
      src.includes('setTimeout') || src.includes('useDeferredValue') || src.includes('debounce');
    expect(hasDebounce, 'SearchBar must implement 300ms debounce').toBe(true);
  });

  it('SearchBar uses useDeferredValue for deferred rendering', () => {
    const src = read('src/components/public/SearchBar.tsx');
    expect(src.includes('useDeferredValue'), 'SearchBar must use useDeferredValue').toBe(true);
  });

  it('SearchBar sets aria-busy during search', () => {
    const src = read('src/components/public/SearchBar.tsx');
    expect(src.includes('aria-busy'), 'SearchBar must have aria-busy during search').toBe(true);
  });

  it('SearchBar clears input on Escape key', () => {
    const src = read('src/components/public/SearchBar.tsx');
    // Should have onKeyDown handler for Escape
    const hasEscapeHandler =
      src.includes('onKeyDown') && (src.includes('Escape') || src.includes('Escape'));
    expect(hasEscapeHandler, 'SearchBar must handle Escape key to clear').toBe(true);
  });

  it('SearchBar passes debounced value to onChange callback', () => {
    const src = read('src/components/public/SearchBar.tsx');
    // onChange should be called with debounced value, not raw input
    expect(src.includes('onChange'), 'SearchBar must call onChange').toBe(true);
  });
});

/** S-JOURNEY-EXT: FaqAccordion single-open, keyboard, live region */
describe('S-JOURNEY-EXT FaqAccordion extended a11y', () => {
  it('FaqAccordion implements single-open mode', () => {
    const src = read('src/components/public/FaqAccordion.tsx');
    // Opening one FAQ should close others
    expect(src.includes('setOpen'), 'FaqAccordion must manage open state').toBe(true);
  });

  it('FaqAccordion answer content has aria-live=polite', () => {
    const src = read('src/components/public/FaqAccordion.tsx');
    expect(
  ...
  it('FaqAccordion supports Escape to close open panel', () => {
    const src = read('src/components/public/FaqAccordion.tsx');
    const hasEscapeHandler =
      src.includes('onKeyDown') && (src.includes('Escape') || src.includes('Escape'));
    expect(hasEscapeHandler, 'FaqAccordion must handle Escape to close').toBe(true);
  });
});
