...
describe('US-2 overdue fastlane — dashboard queue', () => {
  ...
  it('dashboard row shows member_code/title/days-late/Rp + fine preview', () => {
    ...
    expect(src, 'RED: dashboard overdue row missing days-late').toMatch(/hari|telat|days[_-]?late|daysLate/i);
  ...
  it('dashboard deep-links ?overdue=1 + empty state', () => {
    const src = read('src/app/admin/page.tsx');
    expect(src, 'RED: dashboard missing deep link ?overdue=1 to peminjaman').toContain('?overdue=1');
...
describe('US-2 overdue fastlane — peminjaman preselect + 1-click return', () => {
  it('peminjaman preselects ?overdue=1 and sorts due_at asc', () => {
    ...
    expect(src, 'RED: peminjaman must fetch GET /api/loans?overdue=1').toMatch(/overdue.*1|1.*overdue/);
  ...
  it('edge: fine boundary (on-time 0, 1-day Rp1.000)', () => {
    ...
    expect(auth, 'fine logic must define FINE_PER_DAY = 1000').toMatch(
      /FINE_PER_DAY\s*=\s*1000/
    );
  ...
  it('edge: race/boundary 409 CONFLICT exact codes preserved', () => {
    ...
    expect(collective, 'collective PUT already-returned guard missing').toMatch(
      /Sudah dikembalikan/
    );
    expect(single, '[id]/return already-returned guard missing').toMatch(/Sudah dikembalikan/);
  });
});
