...
function rubricScore(): { score: number; notes: string[] } {
  const notes: string[] = [];
  let score = 100;
  const books = read("src/lib/books.ts");
  const settingsFn = books.slice(
    books.indexOf("export async function fetchSettings"),
    books.indexOf("export async function fetchBanners")
  );
  if (!settingsFn.includes("unstable_cache")) {
    score -= 10;
...
describe("Wave2 RED probes", () => {
  it("S-sec-search sanitizeIlike('%_[]\\\\;') escapes vs cleanLike leaves _", () => {
    ...
    for (const f of searchFiles) {
      const src = read(f);
      expect(src.includes("sanitizeIlike"), `S-sec-search GREEN: ${f} must import/use sanitizeIlike`).toBe(true);
      expect(src.includes("cleanLike"), `S-sec-search GREEN: ${f} must not contain weak cleanLike`).toBe(false);
    }
    expect(read("src/lib/books.ts").includes('trim().replace(/[%(),]/g'), "S-sec-search GREEN: books.ts must not contain weak inline sanitizer").toBe(false);
  });
  ...
  it("S-cache fetchSettings wrapped in unstable_cache", () => {
    ...
    const end = src.indexOf("export async function fetchBanners");
    const fn = start >= 0 ? src.slice(start, end >= 0 ? end : undefined) : "";
    expect(
      fn.includes("unstable_cache"),
      "S-cache RED: fetchSettings body has no unstable_cache — settings fetched uncached on every render"
    ).toBe(true);
  });
